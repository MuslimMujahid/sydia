...
Daily notes reuse the same character-window chunker as documents and reuse the same embedding service. They differ in what is indexed and how it is addressed:

- The Tiptap document is the source of truth; the plain text derived from it is what gets chunked and embedded, so a formatting-only edit never changes the index.
- Each chunk is denormalised with the note's owner-local `date`, so a date-windowed search filters and ranks in one query without joining back to the note.
- The index is a derived artefact: writes are synchronous and fast, while indexing is queued and coalesced by a per-note text fingerprint. A save that changes nothing retrievable costs no embedding call.
- A note whose indexing fails keeps a pending flag and is retried by a periodic sweep, so an unavailable embedding provider delays retrieval rather than silently dropping the note from it.
...
\- date (owner-local calendar day, YYYY-MM-DD; unique with user_id)

\- timezone (zone the day was written in)

\- content (Tiptap document JSON; the source of truth)

\- text (plain text derived from content; what is embedded)

\- source_type / source_message_id

\- index_pending / index_signature (index freshness and idempotency)
...
\- user_id / date (denormalised for date-windowed retrieval)
