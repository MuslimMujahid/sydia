...
export class CalendarService {
  ...
  async create(
    ...
    const providerEventId = await this.google.createEvent(
      credentials.accessToken,
      credentials.calendarId,
      input,
    );

    return this.calendars.create(userId, {
      ...input,
      provider: 'google',
      providerEventId,
    });
  ...
  async update(
    ...
    if (existing.provider === 'google' && existing.providerEventId) {
      const credentials = await this.credentials(userId);
      if (!credentials) throw new Error('Google Calendar tidak terhubung.');
  ...
  private async credentials(
    ...
    await this.calendars.updateAccessToken(
      userId,
